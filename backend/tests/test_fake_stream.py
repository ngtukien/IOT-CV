...
def test_khung_la_jpeg_qvga_va_frame_id_tang(clip):
    ...
    assert all(f.framesize == "QVGA" and f.jpeg_quality == 30 for f in frames)
    img = cv2.imdecode(np.frombuffer(frames[0].jpeg, np.uint8), cv2.IMREAD_COLOR)
    assert img.shape[:2] == (FAKE_HEIGHT, FAKE_WIDTH)
...
def test_box_nam_trong_khung_va_di_chuyen():
    boxes = [fake_box(t / 10) for t in range(0, 160)]
    for b in boxes:
        assert 0 <= b.x1 < b.x2 <= FAKE_WIDTH
        assert 0 <= b.y1 < b.y2 <= FAKE_HEIGHT
        assert b.label == "fake"
...
def test_box_chay_vong_chu_nhat_va_quay_ve_cho_cu():
    b0, b_nua = fake_box(0.0), fake_box(4.0)
    assert (b0.x1, b0.y1) != (b_nua.x1, b_nua.y1)
    assert fake_box(8.0) == b0  # chu kỳ 8 s
